...
/**
 * A named collection of spells unlocked by a specific perk (e.g. Mage, Cyberwitch).
 */
@Entity
@Table(name = "torg_spell_list")
@Access(AccessType.FIELD)
@Getter
@Setter
@NoArgsConstructor
public class SpellList extends TorgEntity {

    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "torg_spell_list_products", joinColumns = @JoinColumn(name = "list_id"))
    @Column(name = "product")
    private Set<String> products = new HashSet<>();

    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "torg_spell_list_cosms", joinColumns = @JoinColumn(name = "list_id"))
    @Column(name = "cosm", length = 128)
    private Set<String> cosms = new HashSet<>();

    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "torg_spell_list_unlocking_perks", joinColumns = @JoinColumn(name = "list_id"))
    @Column(name = "perk_slug", length = 128)
    private Set<String> unlockingPerks = new HashSet<>();

    @ElementCollection
    @CollectionTable(name = "torg_spell_list_entries", joinColumns = @JoinColumn(name = "list_id"))
