...
public interface ThreatRepository extends JpaRepository<Threat, UUID> {
    ...
    @Query("SELECT t FROM Threat t WHERE :cosm MEMBER OF t.cosms")
    List<Threat> findByCosm(@Param("cosm") String cosm);

    List<Threat> findByUnique(boolean unique);

    List<Threat> findByClearanceLevel(ClearanceLevel clearanceLevel);

    @Query("SELECT t FROM Threat t WHERE :product MEMBER OF t.products")
    List<Threat> findByProduct(@Param("product") String product);

    @Query("SELECT t FROM Threat t WHERE :cosm MEMBER OF t.cosms AND :product MEMBER OF t.products")
