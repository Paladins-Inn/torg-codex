...
public class MiracleList extends TorgEntity {
    ...
    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "torg_miracle_list_cosms", joinColumns = @JoinColumn(name = "list_id"))
    @Column(name = "cosm", length = 128)
    private Set<String> cosms = new HashSet<>();

    @ElementCollection(fetch = FetchType.EAGER)
    @CollectionTable(name = "torg_miracle_list_unlocking_perks", joinColumns = @JoinColumn(name = "list_id"))
