...
This is implemented as four Maven reactor modules — `torg-codex-domain` (framework-independent domain models and events), `torg-codex-application` (driving/driven ports and use-case services), `torg-codex-data` (outbound/driven adapters: persistence, DriveThruRPG HTTP, domain-event bridge), and `torg-codex` (inbound/driving adapters: REST controllers and the composition root) — with a strict one-directional dependency chain from `torg-codex`/`torg-codex-data` towards `torg-codex-application` towards `torg-codex-domain`, continuously enforced by an ArchUnit test suite as part of the standard build. See xref:04_solution_strategy.adoc[] and xref:05_building_block_view/index.adoc[].
|===
