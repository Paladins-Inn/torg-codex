...
As example we show the domain of {system} but the same pattern is applied to every other domain with data lead, too.
...
. The java library `de.paladins-inn.torganized-play:pandurart-store` needs to be added to the dependency list of the client.
The `de.paladins-inn.torganized-play:pandurart-client` will be automatically included.
...
together {
  ...
  component "pandurart" as scsUser << External >>
}
...
. The java library `de.paladins-inn.torganized-play:pandurart-client` needs to be added to the dependency list of the client.
